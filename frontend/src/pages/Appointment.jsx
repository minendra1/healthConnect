...
const Appointment = () => {
  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      let month = date.getMonth() + 1; //increase 1 before jan start from 0
          ...
          <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
            ...
            <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
              {
                docSlots.length && docSlots.map((item, index) => (
                  <div key={index} onClick={() => setSlotIndex(index)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-blue-600 text-white' : 'border border-gray-300'}`}>
            ...
            <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
              {docSlots.length && docSlots[slotIndex].map((slot, index) => (
                <p onClick={() => setSlotTime(slot.time)} key={index} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${slot.time === slotTime ? 'bg-blue-600 text-white' : 'border border-gray-300'}`}>
            ...
            {/* directly calling bookAppointment function on button click */}
          </div>
